...
const { album, track, index } = defineProps<{ album: Album, track: AlbumTrack, index: number }>()

const useiTunes = ref(sharedStore.state.useiTunes)

const song = computed(() => songStore.guess(track.title, album))
const tooltip = computed(() => song.value ? 'Click to play' : '')

const iTunesUrl = computed(() => {
  return `${window.BASE_URL}itunes/song/${album.id}?q=${encodeURIComponent(track.title)}&api_token=${auth.getToken()}`
})
